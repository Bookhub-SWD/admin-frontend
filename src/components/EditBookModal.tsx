...
const EditBookModal: React.FC<EditBookModalProps> = ({ isOpen, onClose, onSuccess, bookId, initialData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        keyword: formData.keyword.split(',').map((k: string) => k.trim()).filter((k: string) => k),
      ...
      console.error('EditBookModal: Error submitting form', err);
      enqueueSnackbar(err.response?.data?.message || 'Cập nhật sách thất bại', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-oxford-blue/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-parchment w-full max-w-2xl max-h-[90vh] rounded-academic shadow-2xl flex flex-col overflow-hidden border border-oxford-blue/10">
        ...
        <div className="p-6 border-b border-oxford-blue/10 flex justify-between items-center bg-white/50">
          <div>
            <h2 className="text-2xl font-serif font-black text-oxford-blue tracking-tight uppercase">Cập nhật </h2>
            <p className="text-xs font-mono font-black text-brass uppercase tracking-widest mt-1">Sửa thông tin sách</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-mono font-black text-oxford-blue/60 uppercase tracking-widest flex items-center gap-2">
              <Info className="h-3 w-3 text-brass" /> Tóm tắt / Mô tả
            </label>
            <textarea
              name="description"
